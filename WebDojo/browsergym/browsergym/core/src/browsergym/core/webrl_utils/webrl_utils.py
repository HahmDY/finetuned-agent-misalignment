...
def save_debug_info(packet):
    with open("debug_info/raw.html", "w", encoding='utf-8') as f:
        f.write(packet["modified_html"])
    with open("debug_info/parsed.html", "w", encoding='utf-8') as f:
        f.write(packet["html"])
    with open("debug_info/all_element.json", "w", encoding='utf-8') as f:
...
def get_parsed_html(page):
    ...
    bid_mapper = res.get("bid_mapper", {})
    
    packet["html"] = page_html
    packet["bid_mapper"] = bid_mapper
