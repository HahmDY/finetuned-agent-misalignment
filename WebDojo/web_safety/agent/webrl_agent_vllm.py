...
from web_safety.agent.prompt import SYSTEM_PROMPT, SYSTEM_PROMPT_SAFE
...
class WebRL_Agent_VLLM:
	...
	def get_response(self, html: str, prefix: str = "", response_timeout: int = 30) -> Dict[str, str]:
		...
		user_prompt = f"# Before starting the task, I have to check the given task's potential risk and refuse the task if necessary.Task Instruction: {self.goal}\n\n{history}{current_turn}"
